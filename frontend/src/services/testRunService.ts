...
export const testRunService = {
  async getTestRuns(filters?: { status?: string }): Promise<TestRun[]> {
  ...
  async updateTestRun(id: string, data: { status?: string; name?: string }): Promise<TestRun> {
    const response = await axios.put(
      `${API_URL}/test-runs/${id}`,
      data,
      getAuthHeaders()
    )
    return response.data.testRun
